...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # this is a two layer network so we must have 2 processes to go through 
        # thus we must go through two sets of matrix multiplication np.dot()
        # we need to progress from the input layer to  the hidden layer
        hidden_layer = np.maximum(0,np.dot(X,W1) + b1)

        # and then from the hidden layer to the ouptlayer otherwise known as the output layer
        scores = np.dot(hidden_layer,W2) + b2
        
        # that is why the two lines above require their own set of W and b values
        ...
        # For this part we are finding the loss
        # The loss is a measurement to verify that the correct class has the highest score
        ...
        # Using this strategy we can determine better parameters to get a more optimal loss (ideally 0)

        dscores = probs
        dscores[range(num_examples),y] -= 1
        dscores /= num_examples

        # Take hidden layer backpropagated from the output layer
        grads['W2'] = np.dot(hidden_layer.T, dscores)
        grads['b2'] = np.sum(dscores, axis = 0 , keepdims=True)

        # next backprop back into the Input layer
        dhidden = np.dot(dscores, W2.T)
        # backprop the ReLU non-linearity
        dhidden[hidden_layer <= 0] = 0

        # finally we can retrieve the required params to be used at the input layer 
